...
##from marcos runner-----------------------------------------------------------------------------------
def solve_greedy( marcosList):
    print("inicia resolucion mediante intelligent greedy",datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
    salida = []

    marcosList = sorted(marcosList, key=lambda x: x[-1])
    #print(marcosList)
    while len(marcosList)>0:
        workerBest,positionBest,costBest=marcosList[0]
        salida.append(([workerBest,positionBest,costBest]))

        listToRemove=[(worker,position,cost) for worker,position,cost in marcosList if (positionBest == position or workerBest==worker)]
        for remover in listToRemove:
            marcosList.remove(remover)
...
outvector=solve_greedy(marcosList)

tablatrabajos=computeJObsData(outvector,W,T,  REQ, Q, LH, LT,R, CG,datosMS)
createCSVout(tablatrabajos,"OutRPO_Positions_greedy")
tablaVisual=computeVisualization(tablatrabajos,T)
createCSVout(tablaVisual,"OutRPO_PositionsVisual_greedy")
tablaKPI=computeKPI(tablatrabajos,R,W,J,hglobal)
createCSVout(tablaKPI,"OutRPO_KPI_greedy")
